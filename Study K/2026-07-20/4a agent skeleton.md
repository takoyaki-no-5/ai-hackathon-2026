...
Agent チャットにコピペ（作業ディレクトリ＝リポ root）:

```
このリポで Next.js App Router + TypeScript + pnpm の最小 Agent 骨格を作って。
要件:
- Vercel AI SDK（ai + @ai-sdk/openai + @ai-sdk/react）+ Zod
- app/api/chat/route.ts で streamText + tool 最低1つ（echoTopic）
- app/page.tsx で useChat、tool 呼び出しが画面に見える
- キーは secrets/.env（OPENAI_API_KEY / OPENAI_MODEL）。コードにベタ書きしない
- .gitignore に .env* と secrets/.env
既存ファイルを壊さない。完了後 pnpm dev で確認手順を書いて。
```

詰まったらエラー全文をそのまま貼る。尖り（画像）は入れない。
...
次: [[Study K/2026-07-20/4b vercel url|#4b Vercel 公開 URL]]
